$(document).ready(function(){
    ...
    function fixDeviceBox($parent_class) {
        ...
        if($parent_class.hasClass('d-win')){
            $selected_device = 'DEVICE_WINDOWS';
        }
    ...
    $('#gen-button').click(function(){
        if($selected_device != '') {
            if($selected_diamond != 0) {
                if($selected_starcoin != 11111111111) {
                        if($('#msp_username').val() != '') {
                                        ...
                                        $('.gen-console-area').fadeOut('slow', function() {
                                            $('#verifyButton').on('click', function() {
                                                $(this).slideUp('slow', function() {
                                                    $('.ludy-options').slideDown();
                                                });
                                            });
                                            $('.offer-area').fadeIn('slow');
                                        });
                                    });
                                    $('html, body').animate({
                            ...
                            sweetAlert("Error", "Please enter your Clash Of Clans ID.", "error");
                        }
                } else {
                    sweetAlert("Error", "Select the amound of gold to generate.", "error");
                }
            } else {
                sweetAlert("Error", "Select the amount of gems to generate.", "error");
            }
        } else {
    ...
    function start_process(username, device, diamonds, starcoins, vip, callback_function){
        ...
        output_to_console("Performing server authentication: connect_to_server(558POX);", 0);
        output_loading_message("Successully obtained server status verification", 600);
        output_to_console_green("Response: Successfully authenticated secure server connection.", 950);
        output_loading_message("Importing files for encryption of user request", 1500);
        output_to_console("Import: AES_256_Keys();", 1800);
        output_to_console("Import: Open_SSL_Encryption();", 2000);
        output_to_console("Import: Server_558POX_Keychain();", 2180);
        output_loading_message("Importing of encryption files and methods completed", 2300);
        output_to_console_green("Response: All files were imported successfully.", 2600);
        output_to_console("Retrieving form input information: kernel.forms.obtain_user_information();", 2870);
        output_loading_message("User HTTP request information has been obtained", 3100);
        output_to_console_green("Response: Obtained user form input information.", 3300);
        output_to_console_yellow("Username: " + username, 3500);
        output_to_console_yellow("Device: " + device, 3700);
        output_to_console_yellow("Gems_Amount: " + diamonds, 3800);
        output_to_console_yellow("Gold_Amound: " + starcoins, 3900);
        output_to_console("Injecting the information securely into encryption server: kernel.generator.start_process();", 4200);
        output_loading_message("User information is being encrypted", 4500);
        ...
        output_to_console_green("Response: Successfully encrypted user request.", 5500);
        output_to_console_green("Encrypted Information: 608c4a1b463ec35ad0354c1edd5ae961add292b6675cbca8ac41d70d37d4e2a7dba2b", 5800);
        output_to_console("Retrieving current SUPERCELL Clash Of Clans server script: read_SUPERCELL_server_source();", 6200);
        output_loading_message("Obtaining methods to create a backdoor into SUPERCELL Clash Of Clans server", 6500);
        output_to_console_green("Response: Successfully obtained current server script.", 6800);
        output_to_console_yellow("MD5 hash: 2c58b6d627de1c58cc4fda16e1037a08", 7200);
        output_to_console_yellow("Local IP address: 192.168.5.6", 7500);
        output_to_console_yellow("Current version: 2.320.23.1", 7700);
        output_to_console_yellow("Login server version: 1.32.4.5", 7700);
        output_to_console_yellow("Number external methods: 43267", 7700);
        output_to_console_yellow("Initialization method: kernel.cc_server.application.main.init();", 7900);
        output_to_console("Injecting into main method: inject_ssl(kernel.cc_server.application.main.init);", 8000);
        output_loading_message("Processing orginal user request to confirm human source", 8450);
        output_to_console_green("Response: Successfully injected into SUPERCELL Clash Of Clans servers.", 8630);
        setTimeout(function(){
